...
public class UIToggle : UIWidgetContainer
{
	...
	public bool activateAlphaDecrement = false;
    ...
    void Start ()
    {
        ...
        if (!Application.isPlaying)
        {
			...
			if (Application.isPlaying && activeSprite != null && activateAlphaDecrement)
    ...
    void Set (bool state)
    {
        ...
        else if (mIsActive != state)
        {
            ...
            {
				if (instantTween && activateAlphaDecrement)
                {
                    activeSprite.alpha = mIsActive ? 1f : 0f;
                }
				else if(activateAlphaDecrement)
                {
