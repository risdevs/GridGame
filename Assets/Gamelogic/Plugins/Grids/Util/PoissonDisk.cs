	...
	*/
	public class PoissonDisk
	{
		...
		public static List<Vector2> GeneratePoisson(Rect rect, float minDist, int newPointsCount)
		{
			...
			var grid = RectGrid<Vector2?>.Rectangle(
				Mathf.CeilToInt(rect.width/cellSize), //grid width
				Mathf.CeilToInt(rect.height/cellSize)); //grid height

			var map = new RectMap(Vector2.one*cellSize)
				.WithWindow(rect)
				.Stretch(grid);
